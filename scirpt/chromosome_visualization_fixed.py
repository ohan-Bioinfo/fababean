...
# Plot 2: Chromosome Distribution (ONLY chr1-chr6)
import subprocess

# Get chromosome counts for main chromosomes only
result = subprocess.run(["cut", "-f1", "03_LD_Prune/Faba_chrOnly_pruned.bim"], 
                       capture_output=True, text=True)
chromosomes = result.stdout.strip().split('\n')

# Filter only chr1-chr6
main_chromosomes = ['chr1', 'chr2', 'chr3', 'chr4', 'chr5', 'chr6']
chrom_counts = {}
...
sorted_chroms = sorted(chrom_counts.items(), key=lambda x: int(x[0][3:]))
...
total_main_snps = sum(chrom_snps)
for bar, value, chrom in zip(bars_chrom, chrom_snps, chrom_names):
    height = bar.get_height()
    percentage = (value / total_main_snps) * 100
...
plt.tight_layout()
plt.savefig('Faba_Bean_Chromosomes_Only.png', dpi=300, bbox_inches='tight')
plt.show()

print("✓ Chromosome visualization (chr1-chr6 only) saved as 'Faba_Bean_Chromosomes_Only.png'")

# Print summary
print(f"\n📊 MAIN CHROMOSOME SNP DISTRIBUTION:")
for chrom, count in sorted_chroms:
    pct = (count / total_main_snps) * 100
    print(f"  {chrom}: {count:,} SNPs ({pct:.1f}%)")
print(f"  Total on main chromosomes: {total_main_snps:,} SNPs")

# Also create heterozygosity plot separately
