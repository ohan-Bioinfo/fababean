...
# Create a custom sorting function for chromosome names
def chrom_sort_key(chrom_name):
    """Sort chromosomes logically: chr1, chr2, ..., chr1L, chr1S, etc."""
    if chrom_name.startswith('chr'):
        # Extract the number part and any letter part
        import re
        match = re.match(r'chr(\d+)([A-Z]*)', chrom_name)
        if match:
            number = int(match.group(1))
            letter = match.group(2) if match.group(2) else ''
            # Return tuple (number, letter) for sorting
            return (number, letter)
    # For non-matching patterns, put at the end
    return (999, chrom_name)

# Sort chromosomes using our custom function
sorted_chroms = sorted(chrom_counts.items(), key=lambda x: chrom_sort_key(x[0]))
...
print(f"\n📊 CHROMOSOME SNP DISTRIBUTION (Final LD-pruned):")
